...
- Move focus to the error summary at the top of the form so that the errors will be announced to screen readers. **This step is important**. It avoids a scenario where screen reader users would have to navigate all the way back through a page to get to the error summary where the errors would then be announced.
